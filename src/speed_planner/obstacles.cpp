...
namespace speed_profile {
    ...
    Obstacles::Obstacles(const SpeedProfileConf &speed_profile_conf) :
            speed_profile_conf_(speed_profile_conf) {
        rrt_conf_ = speed_profile_conf_.rrt();
    }

    void Obstacles::SetObstacles(const planning::PredictionObstacles &obstacle_map) {
        for (int i = 0; i < obstacle_map.obstacle_size(); ++i) {
            obstacles_.push_back(obstacle_map.obstacle(i));
        }
    }

    bool Obstacles::CollisionFree(const planning::speed_profile::Node &parent_node,
